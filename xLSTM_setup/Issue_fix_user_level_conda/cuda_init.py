...
## replace xlstm/blocks/slstm/src/cuda_init.py
env_path = 'home/path/molgraph-xlstm'
#local conda env work around (there is probably a better way..)
...
def load(*, name, sources, extra_cflags=(), extra_cuda_cflags=(), **kwargs):
    ...
    # Add include paths with -I (not isystem)
    extra_cflags.extend([
        f"-I/{env_path}/targets/x86_64-linux/include",
        f"-I/{env_path}/include",
    ])
    ...
    # Add random waiting time to minimize deadlocks due to multi-compiling issues with PyTorch extensions
    time.sleep(random.random() * 10)
